    ...
    <>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-white border-b px-4 h-14 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          <Link href="/admin/settings" className="inline-flex items-center justify-center h-9 w-9 rounded-md text-muted-foreground hover:text-gray-900 hover:bg-gray-100 transition-colors" title="Configuración">
            <Settings className="h-4 w-4" />
          </Link>
          <Button variant="ghost" size="icon" onClick={handleSignOut} title="Cerrar sesión" className="h-9 w-9 text-muted-foreground hover:text-destructive">
      ...
      {/* Mobile bottom nav */}
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t px-2 h-16 flex items-center justify-around safe-area-pb">
        {bottomNav.map((item) => {
